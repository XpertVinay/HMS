...
@section('title', 'Global Roles & Permissions Matrix')
...
<div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 p-6 mb-6">
    <div class="flex justify-between items-center mb-6">
        ...
        <div>
            <form action="{{ route('super_admin.roles_permissions.index') }}" method="GET" class="flex items-center gap-2">
                <label for="organization_id" class="text-sm font-medium text-gray-700 dark:text-gray-300">Context:</label>
                <select name="organization_id" id="organization_id" class="form-control rounded-lg border-gray-300 dark:border-slate-600 dark:bg-slate-700 py-1" onchange="this.form.submit()">
                    <option value="global" {{ $organizationId === 'global' ? 'selected' : '' }}>Global Roles</option>
    ...
    <form action="{{ route('super_admin.roles_permissions.matrix.update') }}" method="POST">
        @csrf
        ...
        <div class="overflow-x-auto">
            ...
            <small class="text-gray-500 mt-2 block">* Super Admin has full control over all matrices.</small>
        </div>
...
<div class="modal fade" id="createRoleModal" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <form action="{{ route('super_admin.roles_permissions.roles.store') }}" method="POST" class="modal-content bg-white dark:bg-slate-800 rounded-xl">
            @csrf
            ...
            <div class="modal-body p-4">
                ...
                <div class="form-group mb-4">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Assign to Organization</label>
                    <select name="organization_id" class="form-control w-full rounded-lg border-gray-300 dark:border-slate-600 dark:bg-slate-700">
...
<div class="modal fade" id="createPermissionModal" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <form action="{{ route('super_admin.roles_permissions.permissions.store') }}" method="POST" class="modal-content bg-white dark:bg-slate-800 rounded-xl">
            @csrf
            <div class="modal-header border-b border-gray-100 dark:border-slate-700 p-4">
                <h5 class="modal-title font-bold">Create New Global Permission</h5>
                <button type="button" class="close text-gray-400 hover:text-gray-600" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body p-4">
                <div class="form-group mb-4">
                    ...
                    <small class="text-gray-500 mt-1 block">Permissions created by Super Admin are global.</small>
                </div>
            </div>
            <div class="modal-footer border-t border-gray-100 dark:border-slate-700 p-4">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
