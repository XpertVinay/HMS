...
Class Action {
	...
	function save_maintenance(){
		...
		$maintenance_date = $maintenance_date.'-01';
		...
		if(empty($id)){
			...
			if($save){
				...
				foreach($amount as $k => $v){
					...
					$ins[] = $this->db->query("INSERT INTO maintenance_items set ".$data);
				}
			...
			if($save){
				foreach($amount as $k => $v){
					...
					if(isset($previous_reading[$k]))
					$data.=", previous_reading = '$previous_reading[$k]' ";
					$ins[] = $this->db->query("UPDATE maintenance_items set ".$data." where type='$k' and maintenance_id = '$id' ");
				}
				return json_encode(array("status"=>1,"id"=>$id));
			}
		}
	...
	function delete_maintenance(){
		...
		$delete2 = $this->db->query("DELETE FROM maintenance_items where maintenance_id = ".$id);
		if($delete && $delete2){
				return 1;
			}
	}
